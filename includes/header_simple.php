<?php
// Simple header include for pages that need to process before output
// Assumes database connection and auth are already included

// Get current user if logged in (if not already set)
...
<head>
  ...
  <title><?php echo e($pageTitle ?? 'ServiceLink • Find Trusted Local Services'); ?></title>
      ...
      <div class="flex items-center justify-between h-16">
       ...
       <a href="<?php echo BASE_URL; ?>/index.php" class="flex items-center space-x-2 text-xl font-bold text-primary-600 hover:text-primary-700 dark:text-primary-400 dark:hover:text-primary-300 transition-colors">
        ...
        <nav class="hidden md:flex items-center space-x-8">
          ...
          <?php if ($currentUser): ?>
            <?php if ($currentUser['role'] === 'provider' || $currentUser['role'] === 'admin'): ?>
